...
def test_explicit_file_produces_function_outline(tmp_path: Any) -> None:
    source = tmp_path / "auth.py"
    source.write_text("def login(username: str) -> bool:\n    return True\n")

    pack = build_context_pack(
        "fix login bug", project=tmp_path, files=["auth.py"], budget=1000
    )
...
def test_budget_is_enforced(tmp_path: Any) -> None:
...
def test_missing_semantic_index_does_not_crash(tmp_path: Any) -> None:
...
def test_changed_mode_handles_no_git_repo_gracefully(tmp_path: Any) -> None:
    (tmp_path / "app.py").write_text("def main():\n    return 1\n")
...
def test_secret_looking_files_are_excluded(tmp_path: Any) -> None:
...
def test_tldrignore_excludes_explicit_file_and_fallback_tree(tmp_path: Any) -> None:
...
def test_tldrignore_excludes_text_search_matches(tmp_path: Any) -> None:
    (tmp_path / ".code-briefcaseignore").write_text("ignored.py\n")
    (tmp_path / "ignored.py").write_text("def ignored_target():\n    return True\n")
...
def test_changed_mode_includes_untracked_files(tmp_path: Any) -> None:
