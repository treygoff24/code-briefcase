...
- **2026-05-16 (dogfooded)** - Single-file TS/TSX diagnostics initially produced false project-alias errors in `llm-council` because raw `tsc <file>` ignored project config. The robust fix is an ephemeral one-file tsconfig that extends the nearest real config and filters output to the target. Project-level JavaScript diagnostics also use an ephemeral JS-only config so TypeScript 6 checks `.js`/`.jsx` files reliably. Verified by `scripts/dogfood_diagnostics.py` against a synthetic oxc repo plus `llm-council` and `prospera-radar-build/web`.
