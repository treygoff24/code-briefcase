...
from code_briefcase.api import extract_file, get_file_tree, search
from code_briefcase.dirty_flag import get_dirty_files
...
def _semantic_files(project: Path, query: str, language: str) -> list[Path]:
    index = project / ".code-briefcase" / "cache" / "semantic" / "index.faiss"
    if not index.exists() or not query:
        return []
    try:
        from code_briefcase.semantic import semantic_search
