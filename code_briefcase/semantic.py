...
PROJECT_ROOT_MARKERS = [".git", "pyproject.toml", "package.json", "Cargo.toml", "go.mod", ".code-briefcase"]
...
def _confirm_download(model_key: str) -> bool:
    ...
    print("   (Set CODE_BRIEFCASE_AUTO_DOWNLOAD=1 to skip this prompt)\n", file=sys.stderr)
...
def _detect_project_languages(project_path: Path, respect_ignore: bool = True) -> List[str]:
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        dirs[:] = [d for d in dirs if d not in {'.git', 'node_modules', '.code-briefcase', 'venv', '.venv', '__pycache__', '.idea', '.vscode', 'env', '.env', 'vendor', 'deps', '_build', 'cover'}]
